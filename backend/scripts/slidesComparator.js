...
async function loadAuth() {
  ...
  if (fs.existsSync(TOKEN_PATH)) {
    ...
    console.log("🔐 Autoriza en:", authUrl);
...
async function exportarSlidesComoImagenes(presentationId) {
  ...
  console.log(`📄 Exportando ${presentation.slides.length} diapositivas`);
  ...
  for (const tipo of tipos) {
    for (const medida of medidas) {
      ...
      try {
        ...
        console.log(`📸 Captura de ${tipo}/${medida}`);
      } catch (err) {
        console.warn(`❌ Error en captura de ${tipo}/${medida}: ${err.message}`);
      }
...
async function compararVisualmente() {
  ...
  for (const img of screenshots) {
    ...
    if (!match) {
      console.warn(`⚠️ No hay slide para ${img}`);
      continue;
    }
    ...
    if (diffPixels > 0) {
      console.log(`❗ Diferencias en ${img} (${diffPixels} pixeles)`);
    } else {
      console.log(`✅ Coincidencia exacta: ${img}`);
    }
  }
}

module.exports = async function () {
  const PRESENTATION_ID = '1GY9ePNT0uoYU3VRL39SvYXHcttobBWy4dvW646pbfHQ'; // tu presentación
